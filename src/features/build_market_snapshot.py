...
def build_market_snapshot(markets: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for market in markets:
        rows.append(
            ...
            "event_title": market.get("events", [{}])[0].get("title") if market.get("events") else None,
    ...
    df["market_age_days"] = (df["updated_at"] - df["created_at"]).dt.total_seconds() / 86400
    df["time_to_resolution_days"] = (df["end_date"] - df["updated_at"]).dt.total_seconds() / 86400
